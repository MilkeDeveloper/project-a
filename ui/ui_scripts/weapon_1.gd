...
func _on_skill_button_pressed(i: int):
	...
	SkillMenuGlobals.specs_visible = false
	SkillMenuGlobals.curent_primary_skill_selected = true
	inv_panel.hide()
...
func _on_mouse_slot_clicked(slot_index: int):
	...
	if inv_panel.GrimoireInv[slot_index] != null and current_time - last_click <= double_click_time and not ItemGlobals.secondary_skill_active:
		print("double_click")
...
func _on_double_click(_index: int):
	if inv_panel.GrimoireInv[_index] is SpecGrimoireData and SkillMenuGlobals.current_primary_spec_slot_selected:
		SkillMenuGlobals.on_grimoire_clicked.emit(inv_panel.GrimoireInv[_index], _index)
		dragging_preview.hide()
	elif inv_panel.GrimoireInv[_index] is SkillCardData and SkillMenuGlobals.curent_primary_skill_selected:
